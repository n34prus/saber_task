...
float UCpCombatSubsystem::ApplyCombatDamage(AActor* DamageCauser, AActor* Target, const FCombatDamagePacket& DamagePacket)
{
...
void UCpCombatSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	Super::Initialize(Collection);
	OnCombatStateChanged.AddDynamic(this, &UCpCombatSubsystem::OnCombatStateChanged_Implementation);
}

void UCpCombatSubsystem::Deinitialize()
{
	OnCombatStateChanged.RemoveAll(this);
	Super::Deinitialize();
}

bool UCpCombatSubsystem::StartCombat()
{
...
bool UCpCombatSubsystem::StopCombat()
{
...
void UCpCombatSubsystem::ResetCombat()
{
...
bool UCpCombatSubsystem::RegisterMember(AActor* Member)
{
	if (!IsValid(Member)) return false;
	if (IsMemberRegistered(Member)) return true;
	if (auto * HealthComponent = Member->FindComponentByClass<UCpCombatHealthComponent>())
	{
		CombatMembers.Add(Member, HealthComponent);
		HealthComponent->OnDeath.AddDynamic(this, &UCpCombatSubsystem::OnDeath);
...
bool UCpCombatSubsystem::UnregisterMember(AActor* Member)
{
...
bool UCpCombatSubsystem::IsMemberRegistered(AActor* Member)
{
	return CombatMembers.Contains(Member);
}

void UCpCombatSubsystem::SetDifficulty(float NewDifficulty)
{
	Difficulty = FMath::Clamp(NewDifficulty, 0.f, 1.f);
	OnDifficultyChanged.Broadcast(Difficulty);
}

float UCpCombatSubsystem::GetDifficulty()
{
	return Difficulty;
}

void UCpCombatSubsystem::OnCombatStateChanged_Implementation(ECpCombatState NewState)
{
...
void UCpCombatSubsystem::OnDeath(AActor* DeadActor)
{
	if (!DeadActor) return;
...
void UCpCombatSubsystem::CheckFinishCondition()
{
